...
.closing-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:.85rem; margin-bottom:1rem; }
@media(max-width:768px){
  .sidebar { display:none !important; }
  .kpi-grid { grid-template-columns:1fr 1fr !important; }
  .fx-grid, .com-grid { grid-template-columns:1fr 1fr !important; }
  .closing-grid { grid-template-columns:1fr 1fr !important; }
}
</style>
</head>
<body>
...
const INVOICES = [
  { id:"INV-001", client:"BuildCo Ltd", amount:12400, date:"Jun 10", status:"paid", bank:"Banco Galicia", week:"Jun 08 – Jun 14" },
  { id:"INV-002", client:"Metro Supplies", amount:8750, date:"Jun 08", status:"pending", bank:"Banco Santander", week:"Jun 08 – Jun 14" },
  { id:"INV-003", client:"Alpha Group", amount:31200, date:"Jun 05", status:"paid", bank:"Banco Galicia", week:"Jun 01 – Jun 07" },
  { id:"INV-004", client:"City Works", amount:5600, date:"Jun 01", status:"overdue", bank:"Banco Nación", week:"Jun 01 – Jun 07" },
  { id:"INV-005", client:"TechParts SA", amount:19800, date:"May 28", status:"paid", bank:"Banco Santander", week:"May 25 – May 31" },
  { id:"INV-006", client:"Harbor Imports", amount:44100, date:"May 25", status:"pending", bank:"Banco Galicia", week:"May 25 – May 31" },
];
const ALERTS = [
...
let currentUser={name:"Ezequiel Prilusky"}, currentCompany=0, fxRates=null, commodities=null, currentPage="dashboard";
...
async function fetchFXRates(){
  const res=await fetch("https://api.frankfurter.dev/v1/latest?base=USD&symbols=EUR,ARS,BRL,MXN,COP,CLP,GBP,CNY");
...
<p style="color:var(--muted);font-size:.85rem;margin:.2rem 0 1.25rem;">${company.name} · ${company.sector}${fxRate?` <span style="color:var(--accent)">1 USD = ${fxRate.toFixed(2)} ${company.currency}</span>`:''}</p>
<div id="dashCurrencyCards">${renderCurrencyCards()}</div>
<div class="kpi-grid">
...
async function renderCashflow(main,company){
  main.innerHTML=`<div class="fade-in"><h1 style="font-family:'Space Grotesk',sans-serif;font-size:1.5rem;font-weight:700;">Cash Flow</h1><p style="color:var(--muted);font-size:.85rem;margin:.2rem 0 1.5rem;">Live projections from your Flask microservice</p><div id="cfContent"><span class="spinner"></span> Fetching from your API…</div></div>`;
  try{
    const forecast=await fetchForecast(company.seed);
...
${MONTHS.slice(0,6).map((m,i)=>`<tr style="border-top:1px solid var(--border)"><td style="padding:.65rem .75rem;font-weight:500">${m}</td><td style="padding:.65rem .75rem">${fmt(company.seed[i]*1000)}</td><td style="padding:.65rem .75rem;color:var(--muted)">${fmt(Math.round((company.seed[5]+forecast.tendencia*(i-5))*1000))}</td><td style="padding:.65rem .75rem"><span class="badge badge-green">On track</span></td></tr>`).join("")}
    ...
    document.getElementById("cfContent").innerHTML=`<div style="color:var(--danger);font-size:.85rem">⚠ ${e.message} — Render may be cold-starting. Reload in 30s.</div>`;
  }
}
function getCurrencyList(){
  return[
    {code:"EUR",flag:"🇪🇺",name:"Euro",rate:fxRates?.EUR?(1/fxRates.EUR).toFixed(4)+" EUR":"—"},
    ...
    {code:"GBP",flag:"🇬🇧",name:"British Pound",rate:fxRates?.GBP?(1/fxRates.GBP).toFixed(4)+" GBP":"—"},
