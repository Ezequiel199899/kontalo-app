...
<div class="ticker-track" id="tickerTrack" style="padding:0 1.5rem;">
<span class="ticker-item"><span class="spinner"></span> Loading live market rates…</span>
</div>
</div>
...
currentCompany = parseInt(document.getElementById("companySelect").value);
...
<div class="nav-item ${currentPage===n.id?'active':''}" onclick="renderPage('${n.id}')">
...
return {
goldUSD: g.status==="fulfilled" ? +(1/g.value.xau.usd).toFixed(2) : null,
silverUSD: s.status==="fulfilled" ? +(1/s.value.xag.usd).toFixed(2) : null,
};
...
if (!fxRates) {
track.innerHTML = '<span class="ticker-item" style="color:var(--danger)">⚠ Live rates unavailable</span>';
return;
}
const items = [
...
<span style="color:var(--faint)">${i.label}</span>
<span style="color:${i.up?'var(--accent)':'var(--danger)'};font-weight:600">${i.val}</span>
<span style="color:${i.up?'var(--accent)':'var(--danger)'};font-size:.65rem">${i.up?'▲':'▼'}</span>
</span>
`).join("");
}
</script><script>
...
<p style="color:var(--muted);font-size:.85rem;margin:.2rem 0 1.75rem;">${company.name} · ${company.sector} ${fxRate?`<span style="color:var(--accent);margin-left:.5rem">1 USD = ${fxRate.toFixed(2)} ${company.currency}</span>`:''}</p>
<div class="kpi-grid">
...
<div class="card" style="padding:1.25rem 1.4rem"><div style="font-size:.7rem;color:var(--faint);text-transform:uppercase;margin-bottom:.5rem">Active alerts</div><div style="font-family:'Space Grotesk',sans-serif;font-size:1.75rem;font-weight:700">2</div></div>
</div>
...
document.getElementById("kpiProjection").innerHTML = `<div style="font-size:.7rem;color:var(--faint);text-transform:uppercase">AI projection</div><div style="font-family:'Space Grotesk',sans-serif;font-size:1.75rem;font-weight:700">${fmt(Math.round(forecast.proyeccion*1000))}</div>`;
document.getElementById("kpiAvg").innerHTML = `<div style="font-size:.7rem;color:var(--faint);text-transform:uppercase">Avg monthly</div><div style="font-family:'Space Grotesk',sans-serif;font-size:1.75rem;font-weight:700">${fmt(Math.round(forecast.promedio*1000))}</div>`;
} catch(e) {
document.getElementById("apiStatus").innerHTML = `<div class="card" style="padding:.9rem;margin-bottom:1rem;color:var(--danger);font-size:.85rem">⚠ ${e.message} — Render may be cold-starting, reload in 30s.</div>`;
}
}

async function renderCashflow(main, company) {
main.innerHTML = `<div class="fade-in"><h1 style="font-family:'Space Grotesk',sans-serif;font-size:1.5rem;font-weight:700;">Cash Flow</h1><div id="cfContent" style="margin-top:1.5rem"><span class="spinner"></span> Fetching…</div></div>`;
try {
const forecast = await fetchForecast(company.seed);
document.getElementById("cfContent").innerHTML = `<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1rem"><div class="card" style="padding:1rem">Average: ${fmt(Math.round(forecast.promedio*1000))}</div><div class="card" style="padding:1rem">Trend: ${forecast.tendencia.toFixed(1)}K</div><div class="card" style="padding:1rem">Next: ${fmt(Math.round(forecast.proyeccion*1000))}</div></div>`;
} catch(e) {
document.getElementById("cfContent").innerHTML = `<div style="color:var(--danger)">⚠ ${e.message}</div>`;
}
}

function renderMarkets(main) {
const currencies = [
{ code:"EUR", flag:"🇪🇺", rate: fxRates?.EUR ? fxRates.EUR.toFixed(4) : "—" },
{ code:"ARS", flag:"🇦🇷", rate: fxRates?.ARS ? fxRates.ARS.toFixed(2) : "—" },
{ code:"BRL", flag:"🇧🇷", rate: fxRates?.BRL ? fxRates.BRL.toFixed(4) : "—" },
{ code:"MXN", flag:"🇲🇽", rate: fxRates?.MXN ? fxRates.MXN.toFixed(4) : "—" },
];
main.innerHTML = `<div class="fade-in"><h1 style="font-family:'Space Grotesk',sans-serif;font-size:1.5rem;font-weight:700;">FX & Markets</h1><div class="fx-grid" style="margin-top:1.5rem">${currencies.map(c => `<div class="fx-card"><span style="font-size:1.4rem">${c.flag}</span><div style="font-weight:700">${c.code}: ${c.rate}</div></div>`).join("")}</div></div>`;
}

function renderInvoices(main) {
main.innerHTML = `<div class="fade-in"><h1 style="font-family:'Space Grotesk',sans-serif;font-size:1.5rem;font-weight:700;">Invoices</h1><div class="card" style="margin-top:1.5rem;padding:1rem">${INVOICES.map(inv => `<div style="padding:.5rem 0;border-bottom:1px solid var(--border)">${inv.id} - ${inv.client} - $${inv.amount} - <span class="badge badge-${inv.status==='paid'?'green':inv.status==='overdue'?'red':'yellow'}">${inv.status}</span></div>`).join("")}</div></div>`;
}

function renderAlerts(main) {
main.innerHTML = `<div class="fade-in"><h1 style="font-family:'Space Grotesk',sans-serif;font-size:1.5rem;font-weight:700;">Alerts</h1>${ALERTS.map(a => `<div class="alert-item alert-${a.type} card" style="margin:1rem 0;padding:1rem">${a.title}<br><span style="color:var(--muted);font-size:.85rem">${a.desc}</span></div>`).join("")}</div>`;
}
</script>
</body>
</html>
