  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:"1.5rem" }}>
      <div style={{ width:"100%", maxWidth:400 }} className="fade-in">
          ...
          {[["Email","email","you@company.com",email,setEmail],["Password","password","••••••••",pass,setPass]].map(([lbl,type,ph,val,set],i) => (
            <div key={lbl} style={{ marginBottom:i===0?"1rem":"1.4rem" }}>
              ...
              <input className="input" type={type} placeholder={ph} value={val}
                onChange={e=>set(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handle()} />
            </div>
          ))}
        ...
        <p style={{ textAlign:"center", marginTop:"1.25rem", fontSize:".78rem", color:G.faint }}>
          No account? <span style={{ color:G.accent, cursor:"pointer" }}>Start free trial →</span>
        </p>
      </div>
    </div>
  );
...
function DashboardPage({ company, fx }) {
  ...
  const chartData = MONTHS.map((m,i)=>({
    month:m,
    actual:    i<6 ? Math.round(company.seed[Math.min(i,5)]*1000) : null,
    projected: forecast ? Math.round((company.seed[5]+forecast.tendencia*(i-5))*1000) : Math.round(company.seed[5]*1000),
  }));

  const latest = company.seed[5]*1000;
  const prev   = company.seed[4]*1000;
  const delta  = (((latest-prev)/prev)*100).toFixed(1);

  // FX context for company currency
    ...
    <div className="fade-in">
      <div style={{ marginBottom:"1.75rem" }}>
        ...
        <p style={{ color:G.muted, fontSize:".85rem", marginTop:".2rem" }}>
          ...
          {fxRate && <span style={{ marginLeft:".75rem", color:G.accent, fontSize:".8rem" }}>1 USD = {fxRate.toFixed(2)} {company.currency}</span>}
        </p>
      </div>
      ...
      <div className="kpi-grid" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"1rem", marginBottom:"1.5rem" }}>
        ...
        <KPI label="AI projection (next)"
          value={loading?"…":error?"Error":fmt(Math.round(forecast.proyeccion*1000))}
          delta={loading?"Calling API…":error?"API offline":`Trend: ${forecast.tendencia>0?"↑":"↓"} ${Math.abs(forecast.tendencia).toFixed(1)}K/mo`}
          up={!error&&forecast?.tendencia>0} loading={loading} />
        <KPI label="Avg monthly (6mo)"
          value={loading?"…":error?"Error":fmt(Math.round(forecast.promedio*1000))}
          delta="From live API" up={true} loading={loading} />
        <KPI label="Active alerts" value="2" delta="1 critical" up={false} />
      </div>

      {error && (
        <div className="card" style={{ padding:".9rem 1.25rem", marginBottom:"1.25rem", borderColor:G.danger+"44", fontSize:".85rem", color:G.danger }}>
          ⚠ {error} — Render free tier may be cold-starting. Wait ~30s and reload.
        </div>
      )}
      ...
      <div className="card" style={{ padding:"1.5rem", marginBottom:"1.25rem" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"1.25rem" }}>
          <div>
            <div style={{ fontFamily:G.fontDisplay, fontWeight:600 }}>Cash flow — 12 months</div>
            <div style={{ color:G.muted, fontSize:".78rem", marginTop:".1rem" }}>Actual + AI projection</div>
          </div>
          <div style={{ display:"flex", gap:"1rem", fontSize:".75rem" }}>
            <span style={{ color:G.accent }}>● Actual</span>
            <span style={{ color:G.accent+"77" }}>● Projected</span>
          </div>
        </div>
          ...
          <AreaChart data={chartData} margin={{ top:5, right:10, left:0, bottom:0 }}>
            <defs>
              <linearGradient id="g1" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%"   stopColor={G.accent} stopOpacity={.3}/>
        ...
        <div className="card" style={{ padding:"1.25rem" }}>
          ...
          {INVOICES.slice(0,4).map(inv=>(
            <div key={inv.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:".6rem 0", borderBottom:`1px solid ${G.border}` }}>
              <div>
                <div style={{ fontSize:".85rem", fontWeight:500 }}>{inv.client}</div>
                <div style={{ fontSize:".72rem", color:G.faint }}>{inv.id} · {inv.date}</div>
              </div>
              <div style={{ textAlign:"right" }}>
                <div style={{ fontSize:".88rem", fontWeight:600 }}>${inv.amount.toLocaleString()}</div>
                <span className={`badge badge-${inv.status==="paid"?"green":inv.status==="overdue"?"red":"yellow"}`}>{inv.status}</span>
              </div>
            </div>
          ))}
        </div>
        <div className="card" style={{ padding:"1.25rem" }}>
          ...
          {ALERTS.map((a,i)=>(
            <div key={i} className={`alert-item alert-${a.type}`}>
              <div style={{ fontSize:".82rem", fontWeight:600, marginBottom:".2rem" }}>{a.title}</div>
              <div style={{ fontSize:".75rem", color:G.muted }}>{a.desc}</div>
              <div style={{ fontSize:".68rem", color:G.faint, marginTop:".3rem" }}>{a.time}</div>
            </div>
          ))}
        </div>
      </div>
...
function CashFlowPage({ company }) {
  ...
  useEffect(() => {
    setLoading(true); setError(null);
    fetchForecast(company.seed).then(setForecast).catch(e=>setError(e.message)).finally(()=>setLoading(false));
  }, [company.id]);

  const n = { "3m":3, "6m":6, "12m":12 }[tab];
  const data = MONTHS.slice(0,n).map((m,i)=>({
    month:m,
    actual:    i<6 ? Math.round(company.seed[Math.min(i,5)]*1000) : null,
    projected: forecast ? Math.round((company.seed[5]+forecast.tendencia*(i-5))*1000) : null,
  }));

  return (
    <div className="fade-in">
      <div style={{ marginBottom:"1.75rem" }}>
        <h1 style={{ fontFamily:G.fontDisplay, fontSize:"1.5rem", fontWeight:700 }}>Cash Flow</h1>
        <p style={{ color:G.muted, fontSize:".85rem", marginTop:".2rem" }}>Live projections from your Flask microservice</p>
      </div>

      {forecast && (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"1rem", marginBottom:"1.25rem" }}>
          {[
            { label:"6-month average",  val:fmt(Math.round(forecast.promedio*1000)),   note:"promedio"    },
            { label:"Monthly trend",    val:`${forecast.tendencia>0?"+":""}${forecast.tendencia.toFixed(1)}K`, note:"tendencia" },
            { label:"Next projection",  val:fmt(Math.round(forecast.proyeccion*1000)), note:"proyeccion"  },
          ].map(c=>(
            <div key={c.label} className="card" style={{ padding:"1rem 1.25rem" }}>
              <div style={{ fontSize:".7rem", color:G.faint, textTransform:"uppercase", letterSpacing:".08em", marginBottom:".4rem" }}>{c.label}</div>
              <div style={{ fontFamily:G.fontDisplay, fontSize:"1.6rem", fontWeight:700, color:G.accent }}>{c.val}</div>
              <div style={{ fontSize:".68rem", color:G.faint, marginTop:".3rem" }}>API: <code style={{ color:G.muted }}>{c.note}</code></div>
            </div>
          ))}
        </div>
      )}
      {loading && <div style={{ color:G.muted, fontSize:".85rem", marginBottom:"1rem", display:"flex", gap:".6rem", alignItems:"center" }}><Spin/> Fetching from your API…</div>}
      ...
      <div className="card" style={{ padding:"1.5rem", marginBottom:"1.25rem" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"1.25rem" }}>
          ...
          <div style={{ display:"flex", gap:".2rem", background:G.bg, borderRadius:8, padding:".2rem" }}>
            {["3m","6m","12m"].map(t=>(
      ...
      <div className="card" style={{ padding:"1.25rem" }}>
        <div style={{ fontFamily:G.fontDisplay, fontWeight:600, marginBottom:"1rem" }}>Monthly breakdown</div>
        <table style={{ width:"100%", borderCollapse:"collapse", fontSize:".85rem" }}>
          <thead>
            <tr style={{ color:G.faint, fontSize:".72rem", textTransform:"uppercase" }}>
              {["Month","Actual","Projected","Variance","Status"].map(h=>(
                <th key={h} style={{ textAlign:"left", padding:".5rem .75rem", fontWeight:500 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {data.map((row,i)=>{
              const v = row.actual&&row.projected ? (((row.actual-row.projected)/row.projected)*100).toFixed(1) : null;
              return (
                <tr key={i} style={{ borderTop:`1px solid ${G.border}` }}>
                  <td style={{ padding:".65rem .75rem", fontWeight:500 }}>{row.month}</td>
                  <td style={{ padding:".65rem .75rem" }}>{row.actual?fmt(row.actual):<span style={{ color:G.faint }}>—</span>}</td>
                  <td style={{ padding:".65rem .75rem", color:G.muted }}>{row.projected?fmt(row.projected):"—"}</td>
                  <td style={{ padding:".65rem .75rem", color:v>0?G.accent:v<0?G.danger:G.faint }}>{v!==null?`${+v>0?"+":""}${v}%`:"—"}</td>
                  <td style={{ padding:".65rem .75rem" }}>
                    {row.actual
                      ? <span className={`badge ${+v>=0?"badge-green":"badge-red"}`}>{+v>=0?"On track":"Below"}</span>
...
function InvoicesPage() {
  ...
  return (
    <div className="fade-in">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"1.75rem" }}>
        <div>
          <h1 style={{ fontFamily:G.fontDisplay, fontSize:"1.5rem", fontWeight:700 }}>Invoices</h1>
          <p style={{ color:G.muted, fontSize:".85rem", marginTop:".2rem" }}>Track and manage your billing</p>
        </div>
          ...
          <thead style={{ background:G.bg }}>
            <tr style={{ color:G.faint, fontSize:".72rem", textTransform:"uppercase" }}>
              {["Invoice","Client","Amount","Date","Status",""].map(h=>(
                <th key={h} style={{ textAlign:"left", padding:".75rem 1.25rem", fontWeight:500 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.map(inv=>(
              <tr key={inv.id} style={{ borderTop:`1px solid ${G.border}` }}
                onMouseEnter={e=>e.currentTarget.style.background=G.bg}
                onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                <td style={{ padding:".9rem 1.25rem", fontWeight:600, color:G.accent }}>{inv.id}</td>
                <td style={{ padding:".9rem 1.25rem" }}>{inv.client}</td>
                <td style={{ padding:".9rem 1.25rem", fontWeight:600 }}>${inv.amount.toLocaleString()}</td>
                <td style={{ padding:".9rem 1.25rem", color:G.muted }}>{inv.date}</td>
                <td style={{ padding:".9rem 1.25rem" }}>
                  <span className={`badge badge-${inv.status==="paid"?"green":inv.status==="overdue"?"red":"yellow"}`}>{inv.status}</span>
                </td>
                <td style={{ padding:".9rem 1.25rem" }}>
                  <button className="btn btn-ghost" style={{ padding:".35rem .8rem", fontSize:".78rem" }}>View</button>
                </td>
              </tr>
            ))}
          </tbody>
...
function AlertsPage() {
  return (
    <div className="fade-in">
      <div style={{ marginBottom:"1.75rem" }}>
        <h1 style={{ fontFamily:G.fontDisplay, fontSize:"1.5rem", fontWeight:700 }}>Alerts</h1>
        <p style={{ color:G.muted, fontSize:".85rem", marginTop:".2rem" }}>Kontalo watches your numbers 24/7</p>
      </div>
      <div style={{ maxWidth:680 }}>
        {ALERTS.map((a,i)=>(
          <div key={i} className={`alert-item alert-${a.type} card`} style={{ marginBottom:".85rem", padding:"1.1rem 1.25rem" }}>
            <div style={{ display:"flex", justifyContent:"space-between" }}>
              <div style={{ fontSize:".9rem", fontWeight:600, marginBottom:".3rem" }}>{a.title}</div>
              <span style={{ fontSize:".7rem", color:G.faint }}>{a.time}</span>
            </div>
            <div style={{ fontSize:".83rem", color:G.muted }}>{a.desc}</div>
          </div>
        ))}
      </div>
    </div>
...
function SettingsPage({ user, company }) {
  ...
  return (
    <div className="fade-in" style={{ maxWidth:580 }}>
      <div style={{ marginBottom:"1.75rem" }}>
        <h1 style={{ fontFamily:G.fontDisplay, fontSize:"1.5rem", fontWeight:700 }}>Settings</h1>
        <p style={{ color:G.muted, fontSize:".85rem", marginTop:".2rem" }}>Account and integrations</p>
      </div>
      <div className="card" style={{ padding:"1.5rem", marginBottom:"1rem" }}>
        <div style={{ fontFamily:G.fontDisplay, fontWeight:600, marginBottom:"1.25rem" }}>Profile</div>
        <div style={{ display:"grid", gap:"1rem", marginBottom:"1.25rem" }}>
          {[["Full name",user.name],["Email",user.email]].map(([lbl,val])=>(
            <div key={lbl}>
              <label style={{ display:"block", fontSize:".78rem", color:G.muted, marginBottom:".35rem" }}>{lbl}</label>
      ...
      <div className="card" style={{ padding:"1.5rem", marginBottom:"1rem" }}>
        <div style={{ fontFamily:G.fontDisplay, fontWeight:600, marginBottom:".6rem" }}>API Connections</div>
        {[
          { label:"Cash flow API (your backend)", url:`${API_BASE}/forecast`, status:"Connected" },
          { label:"FX rates (Frankfurter/ECB)",   url:"api.frankfurter.dev/v1/latest", status:"Connected" },
          { label:"Metals (fawazahmed0 CDN)",      url:"cdn.jsdelivr.net/npm/@fawazahmed0/currency-api", status:"Connected" },
        ].map(s=>(
          <div key={s.label} style={{ marginBottom:"1rem" }}>
            <div style={{ fontSize:".78rem", color:G.muted, marginBottom:".25rem" }}>{s.label}</div>
            <div style={{ background:G.bg, border:`1px solid ${G.border}`, borderRadius:7, padding:".6rem 1rem", fontFamily:"monospace", fontSize:".78rem", color:G.accent }}>{s.url}</div>
            <div style={{ display:"flex", alignItems:"center", gap:".4rem", marginTop:".3rem" }}>
              ...
              <span style={{ fontSize:".7rem", color:G.accent }}>{s.status}</span>
            </div>
          </div>
        ))}
      </div>
      <div className="card" style={{ padding:"1.5rem" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <div style={{ fontFamily:G.fontDisplay, fontWeight:600, marginBottom:".4rem" }}>Current plan</div>
            <span className="badge badge-green" style={{ fontSize:".8rem" }}>{company.plan}</span>
            <p style={{ color:G.muted, fontSize:".82rem", marginTop:".5rem" }}>Renews July 15, 2026</p>
          </div>
...
export default function KontaloApp() {
  ...
  // Global FX state — fetched once, shared across pages + ticker
  ...
  const loadMarkets = useCallback(() => {
    ...
    Promise.allSettled([fetchFXRates(), fetchCommodities()])
      .then(([fxRes, comRes]) => {
        if (fxRes.status==="fulfilled") { setFx(fxRes.value); setFxDate(new Date().toLocaleTimeString()); }
        else setFxError(fxRes.reason?.message || "FX unavailable");
        if (comRes.status==="fulfilled") setCommodities(comRes.value);
      })
      .finally(()=>setFxLoading(false));
  }, []);

  useEffect(()=>{ if(user) loadMarkets(); }, [user]);

  if (!user) return <><style>{css}</style><LoginScreen onLogin={u=>{setUser(u);}}/></>;

  const pages = {
    dashboard: <DashboardPage company={company} fx={fx}/>,
    cashflow:  <CashFlowPage  company={company}/>,
    markets:   <MarketsPage   fx={fx} commodities={commodities} fxLoading={fxLoading} fxError={fxError} fxDate={fxDate} onRefresh={loadMarkets}/>,
    invoices:  <InvoicesPage/>,
    alerts:    <AlertsPage/>,
    settings:  <SettingsPage  user={user} company={company}/>,
  };
    ...
    <>
      ...
      <div style={{ display:"flex", minHeight:"100vh" }}>
        <Sidebar page={page} setPage={setPage} user={user}
          ...
          setCompany={c=>{setCompany(c);setPage("dashboard");}}
          onLogout={()=>setUser(null)}/>
        <div style={{ flex:1, display:"flex", flexDirection:"column", minHeight:"100vh" }}>
          <LiveTicker fx={fx} commodities={commodities} loading={fxLoading}/>
