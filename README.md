...
function getR(c){return mktData.rates[c]||FALLBACK[c]||"—";}
...
const rr=await Promise.allSettled(pp.map(function(p){return fetch("https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=USD&to_currency="+p+"&apikey="+AV_KEY).then(function(r){return r.json();});}));
rr.forEach(function(r,i){if(r.status==="fulfilled"&&r.value["Realtime Currency Exchange Rate"]){mktData.rates[pp[i]]=parseFloat(r.value["Realtime Currency Exchange Rate"]["5. Exchange Rate"]).toFixed(4);}else{mktData.rates[pp[i]]=FALLBACK[pp[i]];}});
}catch(e){}
...
const items=[
{label:"USD/ARS BNA",val:"C $"+FALLBACK.ARS_BNA_C+" V $"+FALLBACK.ARS_BNA_V,up:false},
{label:"USD/ARS Blue",val:"C $"+FALLBACK.ARS_BLUE_C+" V $"+FALLBACK.ARS_BLUE_V,up:false},
...
const res=await fetch(API_BASE+"/forecast",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({values:values})});
if(!res.ok)throw new Error("API "+res.status);
return res.json();
}

async function renderDashboard(main,co){
...
'<div class="card" style="padding:1rem 1.2rem" id="kpiP"><div style="font-size:.68rem;color:var(--faint);text-transform:uppercase;margin-bottom:.4rem">Proyección IA</div><span class="spinner"></span></div>'+
'<div class="card" style="padding:1rem 1.2rem" id="kpiA"><div style="font-size:.68rem;color:var(--faint);text-transform:uppercase;margin-bottom:.4rem">Promedio mensual</div><span class="spinner"></span></div>'+
...
[
{l:"🇦🇷 Dólar BNA",c:FALLBACK.ARS_BNA_C,v:FALLBACK.ARS_BNA_V,va:"+0,66%",up:true},
{l:"🇦🇷 Dólar Blue",c:FALLBACK.ARS_BLUE_C,v:FALLBACK.ARS_BLUE_V,va:"-0,32%",up:false},
{l:"🇦🇷 Dólar MEP",c:FALLBACK.ARS_MEP,v:FALLBACK.ARS_MEP,va:"+0,78%",up:true},
...
{l:"🇨🇱 Peso CLP",c:FALLBACK.CLP,v:FALLBACK.CLP,va:"-0,05%",up:false},
{l:"🇨🇴 Peso COP",c:FALLBACK.COP,v:FALLBACK.COP,va:"+0,10%",up:true},
].map(function(r){return'<div class="fx-row"><span style="font-size:.88rem;font-weight:500;color:var(--text)">'+r.l+'</span><span style="font-size:.9rem;font-weight:700;color:var(--text)">$'+r.c+'</span><span style="font-size:.9rem;font-weight:700;color:var(--text)">$'+r.v+'</span><span style="font-size:.82rem;color:'+(r.up?'var(--accent)':'var(--danger)')+'">'+r.va+'</span></div>';}).join('')+
'</div>'+
...
{n:"🛢️ Petróleo",u:"barril",v:"USD "+FALLBACK.oil},
{n:"🌾 Soja",u:"tonelada",v:"USD "+FALLBACK.soy},
{n:"🌿 Trigo",u:"bushel",v:"USD "+FALLBACK.wheat},
{n:"🌽 Maíz",u:"tonelada",v:"USD "+FALLBACK.corn},
...
try{
const f=await fetchForecast(co.seed);
document.getElementById("kpiP").innerHTML='<div style="font-size:.68rem;color:var(--faint);text-transform:uppercase;margin-bottom:.4rem">Proyección IA</div><div style="font-family:\'Space Grotesk\',sans-serif;font-size:1.6rem;font-weight:700;color:var(--accent)">'+fmt(Math.round(f.proyeccion*1000))+'</div><div style="font-size:.75rem;color:var(--accent)">▲ Tendencia '+f.tendencia.toFixed(1)+'K/mes</div>';
document.getElementById("kpiA").innerHTML='<div style="font-size:.68rem;color:var(--faint);text-transform:uppercase;margin-bottom:.4rem">Promedio mensual</div><div style="font-family:\'Space Grotesk\',sans-serif;font-size:1.6rem;font-weight:700;color:var(--text)">'+fmt(Math.round(f.promedio*1000))+'</div><div style="font-size:.75rem;color:var(--accent)">▲ API en vivo</div>';
}catch(e){
document.getElementById("apiStatus").innerHTML='<div class="card" style="padding:.9rem;margin-top:.5rem;color:var(--danger);font-size:.82rem;border-color:#FF6B6B44">⚠ '+e.message+' — Render iniciando. Recargá en 30s.</div>';
}
}
...
].map(function(r){return'<div class="fx-row"><span style="font-size:.88rem;font-weight:500;color:var(--text)">'+r.l+'</span><span style="font-size:.92rem;font-weight:700;color:var(--text)">$'+r.c+'</span><span style="font-size:.92rem;font-weight:700;color:var(--text)">$'+r.v+'</span><span style="font-size:.82rem;color:'+(r.up?'var(--accent)':'var(--danger)')+'">'+r.va+'</span></div>';}).join('')+
'</div>'+
